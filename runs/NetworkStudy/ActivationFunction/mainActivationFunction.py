...
GAMMA = 0.1
###
ACTIVATION_FUNCTION = 'leaky_relu'  # 'relu', 'leaky_relu', 'prelu', 'elu'
ALPHA = 0.01 # Used for leaky_relu and elu
###

DIRECTORY = f"runs/unet_ActivationFunction_{ACTIVATION_FUNCTION}"
DIRECTORY_CHECKPOINTS = f"runs/unet_ActivationFunction_{ACTIVATION_FUNCTION}/checkpoints"
...
def conv_layer(input_channels, output_channels, activation=ACTIVATION_FUNCTION, alpha=ALPHA):  # Added activation and alpha parameters
    if activation == 'leaky_relu':
        activation_layer = nn.LeakyReLU(negative_slope=alpha)
    elif activation == 'prelu':
        activation_layer = nn.PReLU()
    elif activation == 'elu':
        activation_layer = nn.ELU(alpha=alpha)
    else:
...
class UNet(nn.Module):
    def __init__(self, n_classes=3, activation=ACTIVATION_FUNCTION, alpha=ALPHA):  # Add activation and alpha parameters
        super(UNet, self).__init__()
        
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        # input dim 256x256
        self.down_1 = conv_layer(3, 64, activation, alpha) #128x128
        self.down_2 = conv_layer(64, 128, activation, alpha) #64x64
        self.down_3 = conv_layer(128, 256, activation, alpha) #32x32
        self.down_4 = conv_layer(256, 512, activation, alpha) #16x16
        self.down_5 = conv_layer(512, 1024, activation, alpha) #8x8
        
        self.up_1 = nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=2, stride=2)
        self.up_conv_1 = conv_layer(1024, 512, activation, alpha)
        self.up_2 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=2, stride=2)
        self.up_conv_2 = conv_layer(512, 256, activation, alpha)
        self.up_3 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=2, stride=2)
        self.up_conv_3 = conv_layer(256, 128, activation, alpha)
        self.up_4 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=2, stride=2)
        self.up_conv_4 = conv_layer(128, 64, activation, alpha)
        
        self.output = nn.Conv2d(in_channels=64, out_channels=n_classes, kernel_size=1, padding=0)  # Use n_classes as the number of output channels
        self.output_activation = nn.Softmax(dim=1) # Softmax fornisce una distribuzione di probabilità su più classi, il che è utile per la segmentazione multiclasse.
...
save_model(m, f"unet_model_{ACTIVATION_FUNCTION}.pth")
