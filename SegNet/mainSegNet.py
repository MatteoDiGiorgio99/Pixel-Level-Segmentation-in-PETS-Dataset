...
m = SegNet(n_classes=3)
m.eval()
to_device(m)
summary(m,(3,256,256))   

## Display the first batch of the training dataset.
save_path = os.path.join(DIRECTORY, "segnet_training_progress_images")
...
### If you want to resume training from a checkpoint, uncomment the following line
#checkpoint_path = os.path.join(DIRECTORY_CHECKPOINTS, 'checkpoint_25.pth')
###

print("Starting training...")
train_loop(m, pets_train_dataloader,pets_test_dataloader,(test_pets_inputs, test_pets_targets), optimizer, scheduler, save_path,None) # None or checkpoint_path

#save the final model
save_model(m, f"SegNet_model.pth")
